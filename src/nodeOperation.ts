...
export const updateNodeStyle = function(object) {
  ...
  nodeEle.style.fontWeight = object.style.fontWeight
  this.linkDiv()
...
export const removeNode = function(el) {
  ...
  t.parentNode.remove()
  this.linkDiv()
  console.log(findEle(originSiblingId))
  this.bus.fire('operation', {
    name: 'removeNode',
    obj: nodeObj,
    originSiblingId,
...
export const moveNode = function(from, to) {
  ...
  const index = fromObj.parent.children.findIndex(node => node === fromObj)
  const next = fromObj.parent.children[index + 1]
  const originSiblingId = next && next.id
  console.log(fromObj, originSiblingId)
...
export function updateNodeNotes(object, notes) {
  const oldVal = object.notes
  object.notes = notes
  const nodeEle = findEle(object.id)
  shapeTpc(nodeEle, object)
  this.linkDiv()
...
export const removeNodeSelf = function(el) {
  const nodeEle = el || this.currentNode
  if (!nodeEle) return
  const nodeObj = nodeEle.nodeObj
  // 没有子集则走删除节点方法 并 return
  if (!(nodeObj.children && nodeObj.children.length)) {
    removeNode.call(this, el)
    return
  }
  if (nodeObj.root === true) {
    throw new Error('Can not remove root node')
  ...
  const originSiblingId = next && next.id
  console.log(findEle(originSiblingId))
  removeNodeSelfObj(nodeObj, index)
  this.refresh()
  // this.selectNode(findEle(originSiblingId))
  this.bus.fire('operation', {
    name: 'removeNodeSelf',
    obj: nodeObj,
    originSiblingId,
