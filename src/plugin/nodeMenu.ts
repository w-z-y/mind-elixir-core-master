...
export default function(mind) {
  ...
  //   } else {
  //     // 修复添加标签无法清空的bug
  //     mind.updateNodeTags(mind.currentNode.nodeObj, null)
  //   }
  // }
  ...
  //   } else {
  //     // 修复添加图标无法清空的bug
  //     mind.updateNodeIcons(mind.currentNode.nodeObj, null)
  //   }
  // }
  let state = 'open'
  ...
  mind.bus.addListener('selectNode', function(nodeObj, clickEvent) {
    ...
    if (nodeObj.style) {
      ...
      if (nodeObj.style.color) {
        const colorItem = menuContainer.querySelector(
          '.palette[data-color="' + nodeObj.style.color + '"]'
        )
        colorItem && (colorItem.className = 'palette nmenu-selected')
      }
    }
    // if (nodeObj.tags) {
