...
export const shapeTpc = function(tpc: Topic, nodeObj: NodeObj) {
  ...
  // 支持备注
  if (nodeObj.notes) {
    const notesIcon = $d.createElement('span')
    notesIcon.className = 'notes'
    notesIcon.style.cursor = 'pointer'
    notesIcon.innerText = 'Ξ'
    tpc.appendChild(notesIcon)
  }
...
export function selectText(div: HTMLElement, isLast?: boolean) {
  const range = $d.createRange()
  range.selectNodeContents(div)
  const getSelection = window.getSelection()
  isLast && range.collapse(false)
  if (getSelection) {
